...
export async function listAnalyzingPicks({ date = "", limit = 5 } = {}) {
  const filters = [
    "select=*",
    "status=eq.analyzing",
    "job_id=not.is.null",
  ...
  filters.push(`limit=${Number(limit) || 5}`);
...
export async function runDueAnalyses({ date, force = false } = {}) {
  ...
  const existing = new Set(
    existingRows
      ...
      .map((row) => `${row.game_pk}:${row.matchup_id}`)
  );
  ...
  for (const game of games) {
    ...
    try {
      ...
      const row = {
        ...
        analysis_url: null,
        analysis_text: "",
        notes: "Analysis queued; waiting for Statr job to complete.",
      };
      await upsertPick(row);
      processed.push({ id, matchupId, status: row.status, jobId });
    } catch (err) {
      await upsertPick({
        id,
        date: slateDate,
...
export async function pollQueuedAnalyses({ date = "", limit = 5 } = {}) {
  const apiBase = normalizeStatrApiBase();
  ...
  for (const row of rows) {
    try {
      ...
      if (["completed", "done", "succeeded", "success"].includes(jobStatus)) {
        ...
        const status = signal.market === "none" ? "no_signal" : "pending";
